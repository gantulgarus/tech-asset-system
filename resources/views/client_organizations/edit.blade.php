        ...
        <div class="container">
                ...
                <div class="form-group mb-3">
                    <select id="branch-dropdown" name="branch_id" class="form-select">
                        @foreach ($branches as $branch)
                            <option value="{{ $branch->id }}" {{ $branch->id == $clientOrganization->branch_id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mb-3">
                    <select id="station-dropdown" name="station_id" class="form-select">
                        ...
                        @foreach ($stations as $station)
                            <option value="{{ $station->id }}" {{ $station->id == $clientOrganization->station_id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" id="name" class="form-control" 
                       value="{{ old('name', $clientOrganization->name) }}" required>
            </div>
            <div class="mb-3">
                <label for="output_name" class="form-label">Гаргалгааны нэр</label>
                <input type="text" name="output_name" id="output_name" class="form-control" value="{{ old('output_name', $clientOrganization->output_name) }}" required>
            </div>

            <div class="mb-3">
                <label for="reduction_capacity" class="form-label">Хэрэглээг бууруулах чадал</label>
                <input type="number" step="any" name="reduction_capacity" id="reduction_capacity" class="form-control" 
                       value="{{ old('reduction_capacity', $clientOrganization->reduction_capacity) }}" required>
            </div>
