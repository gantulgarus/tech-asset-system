...
@section('content')
    <form action="{{ route('client-restrictions.update', $clientRestriction->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="container">
                ...
                <div class="form-group mb-3">
                    <select id="branch-dropdown" name="branch_id" class="form-select">
                        @foreach ($branches as $branch)
                            <option value="{{ $branch->id }}" {{ $branch->id == $clientRestriction->branch_id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mb-3">
                    <select id="station-dropdown" name="station_id" class="form-select">
                        ...
                        @foreach ($stations as $station)
                            <option value="{{ $station->id }}" {{ $station->id == $clientRestriction->station_id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="output_name" id="output_name" class="form-control" value="{{ old('output_name', $clientRestriction->output_name) }}" required>
            </div>
